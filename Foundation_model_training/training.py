...
from dataset import get_dataloader  # Updated: use IMUDataset from dataset.py
from mae import MAEforEEG
from trainer import train_one_epoch, evaluate, NativeScalerWithGradNormCount as NativeScaler
...
def main(config):
    ...
    # Freeze the last 16 Vision Transformer blocks of the encoder
    if hasattr(model, 'blocks') and len(model.blocks) >= 16:
        for block in model.blocks[-16:]:
            for param in block.parameters():
                param.requires_grad = False
        print(f"\n🧊 Last 16 encoder blocks frozen (total blocks: {len(model.blocks)})")
    else:
        print(f"\n⚠️ Model has fewer than 16 encoder blocks; no blocks frozen.")
