...
if __name__ == '__main__':
    # Create a random input tensor with shape [batch_size, channels, time_len]
    input_data = torch.randn(2, 128, 512)
    
    # Instantiate the model
    mae = MAEforEEG(time_len=512)
    
    # Run a forward pass; note that this returns the loss, the prediction, and the mask
    loss, pred, mask = mae(input_data, mask_ratio=0.75)
    
    # Print the outputs
    print("Loss:", loss.item())
    print("Pred shape:", pred.shape)  # expected shape: [batch_size, L, p]
    print("Mask shape:", mask.shape)  # expected shape: [batch_size, L]
